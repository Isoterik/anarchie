/*
	...
	Description:
	Starts automated pumping of liquids from the fuel devices.
*/
...
if(!isNil {_vehicle getVariable "mining"}) exitWith {hint localize "STR_NOTF_DeviceIsPumping";}; //Mining is already in process..
...
_item = switch(true) do {
	case (_zone in ["milk_1"]): {"milku"};
	case (_zone in ["oil_1"]): {"oilu"};
	default {""};
};

if(_item == "") exitWith {hint "Du kannst nur Fluessigkeiten damit pumpen!"; life_action_inUse = false;};
...
while {true} do {
	...
	if(isEngineOn _vehicle) exitWith {titleText[localize "STR_NOTF_PumpingStopped","PLAIN"];};
	titleText[localize "STR_NOTF_DevicePumping","PLAIN"];
	_time = time + 27;
	
	//Wait for 27 seconds with a 'delta-time' wait.
	waitUntil {
		if(isEngineOn _vehicle) exitWith {titleText[localize "STR_NOTF_PumpingStopped","PLAIN"]; true};
