...
if(_ctrl == "money") then
{
	...
	if(life_cash < _num) exitWith {hint "Du hast nicht soviel Geld dabei, um das Fahrzeug zu Parken!"};
	if(_index == -1) then
	{
...
{
	if(((_totalWeight select 1) + _itemWeight) > (_totalWeight select 0)) exitWith {hint "Der Lagerraum ist entweder voll, oder hat nicht mehr soviel Platz frei."};

	if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint "Konnte die Gegenstaende aus deinem Inventar nicht einlagern.";};
	_index = [_ctrl,_inv] call fnc_index;
	if(_index == -1) then
	{
		_inv set[count _inv,[_ctrl,_num]];
