...
if (player distance _house > 20) exitWith { hint "Du musst in deinem Haus sein, um einen Schrank oder Regal zu platzieren."; };
if (_item == "storage2" && count (nearestObjects [position _house, ["B_supplyCrate_F"], 5]) > 0) exitWith { hint "Du kannst nur einen Schrank aufbauen."; };
if (_item == "storage1" && count (nearestObjects [position _house, ["Land_Box_AmmoOld_F"], 5]) > 0) exitWith { hint "Du kannst nur ein Regal aufbauen."; };
if (!(_uid in _owners)) exitWith { hint "Dir gehoert dieses Haus nicht und kannst deshalb hier nichts aufbauen."; };

_maxCount = [typeOf _house] call life_fnc_houseContainers;
if (_maxCount == 0) exitWith { hint "In diesem Haus kannst du kein Regal oder Schrank aufbauen."; };
//if (life_donator < 1) exitWith { hint "You must be donator rank 1 to place storage in a home."; };
...
if (_count >= _maxCount) exitWith { hint "Du kannst nicht mehr Regale oder Schraenke im Moment aufbauen."; };
