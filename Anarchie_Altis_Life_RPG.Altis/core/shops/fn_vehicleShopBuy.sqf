...
if(life_cash < _basePrice) exitWith {hint format["Du hast nicht genug Bargeld bei dir um das zu Kaufen.\n\nDas Fehlt dir: $%1",[_basePrice - life_cash] call life_fnc_numberText];};
if(!([_className] call life_fnc_vehShopLicenses) && _className != "B_MRAP_01_hmg_F") exitWith {hint "Du hast die Lizenz nicht!"};
...
if(_spawnPoint == "") exitWith {hint "Ein Fahrzeug blockiert den Spwanpunkt!";};
life_cash = life_cash - _basePrice;
hint format["Du hast %1 fuer $%2 gekauft",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_basePrice] call life_fnc_numberText];
...
switch(playerSide) do {
	...
	case independent: {
		...
		[_vehicle,"med_littlebird",true] spawn life_fnc_vehicleAnimate;
	};
