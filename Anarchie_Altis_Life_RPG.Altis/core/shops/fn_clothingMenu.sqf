...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart"] && playerSide != civilian) exitWith {hint "Du bist kein Zivi!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du bist noch kein ausgebildeter Rebell!"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Du bist kein Polizist!"; closeDialog 0;};
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith { hint "Du brauchst einen Tauchschein!"; closeDialog 0;};
...
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["Du brauchst %1 um hier einzukaufen!",[_var select 0] call life_fnc_varToStr]; closeDialog 0;};
};
...
//CopTexture Hotfix
[true] call life_fnc_updateTexture;