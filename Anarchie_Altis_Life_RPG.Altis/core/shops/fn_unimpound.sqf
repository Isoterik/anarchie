...
if(_veh distance impound_obj < 50) then
{
	...
	if(count _check > 0) exitWith {hint "Es befindet sich bereits ein Fahrzeug dort.";};
	if(_price > life_cash) then
	{
		_price = _price + 200;
		if(_price > life_atmcash) exitWith {hint "Du hast nicht genug Geld dabei oder auf deinem Bankkonto um dein Fahrzeug auszuloesen."};
	...
	hint format["Du hast dein %1 fuer $%3 ausgeloest",_name,[_price] call life_fnc_numberText];
	detach _veh;
